...
mutate(sex = case_when(s07sex==1 ~ "Male",
  ...
  mutate(d12sumres = case_when(d12sumres==1 ~ "Previous positive not on ART",
    ...
    mutate(agegp = case_when(between(s09age,18,24) ~ "18-24",
                           between(s09age,25,34) ~ "25-34",
                           between(s09age,35,44) ~ "35-44",
                           between(s09age,45,54) ~ "45-54",
                           between(s09age,55,64) ~ "55-64",
                           s09age > 64 ~ "65+" ),
...
individual_survey %>% 
  dplyr::select(s72hiv_ever:s86hiv_attend, d12sumres, hiv_art:hiv_combined) %>% 
...
survey <- individual_survey %>% 
  left_join(dplyr::select(household_survey, hh_id, h02cl_id, h44gpsid:gps_acc, total_hhmember:wealth_quantil, h22hh_step))
...
# Individual data
dat <- survey %>% 
  dplyr::select(today, ind_id, s02cl_id, clinic_id, close_clin,
                # Household data
                hh_id, gps_lat, gps_lng, gps_alt, gps_acc, hh_per_room, pov_score, h22hh_step, 
                # Individual data
                s07sex, sex, s09age, agegp, hiv, hiv_combined,
                # Current TB 
