...
fit_list <- c(fit_list_pov, fit_list_rp)
fit_list <- lapply(fit_list, add_criterion, "loo")

loo_comp <- lapply(fit_list, function(fit) loo_compare(fit_base, fit, criterion = "loo"))
loo_comp

mod_wts <- lapply(fit_list, function(fit) model_weights(fit_base, fit))
mod_wts
...
lapply(fit_list, plot_fixeff)

```

```{r tidy_pov_fixeff}

get_model_data(fit_list[[1]], type = "est", terms = "pov_score_scale") %>% 
  bind_rows(get_model_data(fit_list[[2]], type = "est", terms = paste0("wealth_quant",2:6))) %>% 
  bind_rows(get_model_data(fit_list[[3]], type = "est", terms = paste0("wealth_step",2:6))) %>% 
...
get_model_data(fit_list[[1]], type = "est", terms = "pov_score_scale", transform = NULL) %>% 
  bind_rows(get_model_data(fit_list[[2]], type = "est", terms = paste0("wealth_quant",2:6), transform = NULL)) %>% 
  bind_rows(get_model_data(fit_list[[3]], type = "est", terms = paste0("wealth_step",2:6), transform = NULL)) %>%
...
emmeans_pov %>% 
  mean_hdi() %>% 
  ggplot(aes(wealth_quant, y = emmean, ymin = lower.HPD, ymax = upper.HPD)) +
...
bind_rows(
  ...
  bind_rows(lapply(summ_list_rp, 
                   function(summ) summ$random$clustid))) %>% 
  mutate(Model = factor(c("Baseline","PMT score continuous", "PMT score quantile", "Self-assessed wealth",
