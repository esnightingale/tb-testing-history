...
# Posterior predictive checks
y <- dat_nona$test01
yrep <- posterior_predict(fit2a, draws = 100)

# Individual residuals
dat_nona %>%
  add_residual_draws(fit2a, ndraws = 100) %>%
  ggplot(aes(x = .row, y = .residual)) +
  stat_pointinterval()

dat_nona %>%
  add_predicted_draws(fit2a, ndraws = 100) %>%
  make_probability_residuals(.prediction, test01) %>%
  ggplot(aes(sample = .p_residual)) +
  geom_qq(distribution = qunif) +
  geom_abline()
...
map_clustiid
ggsave(here::here(figdir, "cluster_iid_baseline.png"), height = 5, width = 6)

# Combine plots for paper
plot_base_coeffs + map_clustiid
ggsave(here::here(figdir, "plots_baseline.png"), height = 5, width = 12)
