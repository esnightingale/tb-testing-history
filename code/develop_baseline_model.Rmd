...
ggsave(here::here(figdir, "pp_check.png"), height = 4, width = 5)

# Posterior predictive checks
y <- dat_nona$test01
yrep <- posterior_predict(fit0, draws = 100)

# Individual residuals
dat_nona %>%
  add_residual_draws(fit0, ndraws = 100) %>%
  ggplot(aes(x = .row, y = .residual)) +
  stat_pointinterval()

dat_nona %>%
  add_predicted_draws(fit0, ndraws = 100) %>%
...
# Posterior predictive checks
y <- dat_nona$test01
yrep <- posterior_predict(fit1, draws = 100)

# Individual residuals
dat_nona %>%
  add_residual_draws(fit1, ndraws = 100) %>%
  ggplot(aes(x = .row, y = .residual)) +
  stat_pointinterval()

dat_nona %>%
  add_predicted_draws(fit1, ndraws = 100) %>%
  make_probability_residuals(.prediction, test01, n = 1) %>%
  ggplot(aes(sample = .p_residual)) +
  geom_qq(distribution = qunif) +
  geom_abline()
...
# Posterior predictive checks
y <- dat_nona$test01
yrep <- posterior_predict(fit1a, draws = 100)

# Individual residuals
dat_nona %>%
  add_residual_draws(fit1a, ndraws = 100) %>%
  ggplot(aes(x = .row, y = .residual)) +
  stat_pointinterval()

dat_nona %>%
  add_predicted_draws(fit1a, ndraws = 100) %>%
  make_probability_residuals(.prediction, test01, n = 1) %>%
  ggplot(aes(sample = .p_residual)) +
  geom_qq(distribution = qunif) +
  geom_abline()
...
# Posterior predictive checks
y <- dat_nona$test01
yrep <- posterior_predict(fit2, draws = 100)

# Individual residuals
dat_nona %>%
  add_residual_draws(fit2, ndraws = 100) %>%
  ggplot(aes(x = .row, y = .residual)) +
  stat_pointinterval()

dat_nona %>%
  add_predicted_draws(fit2, ndraws = 100) %>%
...
# Prior predictive check
pp_check(fit2a, ndraws = 200)
pp_check(fit2a, type = "stat",stat = "mean", ndraws = 200)
# ggsave(here::here(figdir,"ppc_tstat_baseline.png"), height = 5, width = 5)

# Posterior predictive checks
y <- dat_nona$test01
yrep <- posterior_predict(fit2a, draws = 200)

yrep %>% 
  reshape2::melt() %>% 
  ggplot(aes(value, group = Var1)) +
  geom_density(col = "lightblue", alpha = 0.5) +
  geom_density(data = y)

# Individual residuals
dat_nona %>%
  add_residual_draws(fit2a, ndraws = 200) %>%
  ggplot(aes(x = .row, y = .residual)) +
  stat_pointinterval()

dat_nona %>%
  add_predicted_draws(fit2a, ndraws = 200) %>%
  make_probability_residuals(.prediction, test01) %>%
  ggplot(aes(sample = .p_residual)) +
  geom_qq(distribution = qunif) +
  geom_abline()
...
# Conditional effects
# pdf(here::here(figdir, "condeff_base1.pdf"), height = 5, width = 6)  
condeffs.plot.list <- plot(conditional_effects(fit2a, ndraws = 200), ask = F)
condeffs.plot.list
# dev.off()
...
plot_model(fit2a, 
  ...
  scale_y_continuous(limits = c(-1,2)) -> plot_base_coeffs
...
plot_byarea(fitted_iid, clust, varname = "mean", lab = "Posterior mean", midpoint = 0) +
  labs(title = "Cluster level residuals ",
       subtitle = "Adjusted for age group, sex and HIV status",
       caption = "") -> map_clustiid
plot_byarea(fitted_iid, clust, varname = "sd", lab = "Posterior SD") +
  scale_fill_gradient(low = "white", high = "blue") + 
  labs(caption = "") -> map_clustiid_sd
map_clustiid / map_clustiid_sd
ggsave(here::here(figdir, "cluster_iid_baseline.png"), height = 10, width = 6)

# Combine plots for paper
plot_base_coeffs + (map_clustiid / map_clustiid_sd) + plot_annotation(tag_levels = "A")
ggsave(here::here(figdir, "plots_baseline.png"), height = 8, width = 12)
